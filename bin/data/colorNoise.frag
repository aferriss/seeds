...
vec3 doMagic(vec2 p)
{
    ...
    vec4 on = vec4(0.0,0.0,0.0,0.0);
    float f = func(q, on);
    
	vec3 col = vec3(0.0);
    col = mix( vec3(0.1,0.3,0.6), vec3(0.1,0.5,0.1), f );
    col = mix( col, vec3(0.1,0.7,0.1), dot(on.zw,on.zw) );
    col = mix( col, vec3(0.1,0.7,0.6), 0.1*on.y*on.y );
    col = mix( col, vec3(0.1,0.7,0.1), 0.5*smoothstep(1.2,1.3,abs(on.z)+abs(on.w)) );
    col = clamp( col*f*2.0, 0.0, 1.0 );
    
    
	vec3 nor = normalize( vec3( dFdx(f)*iResolution.x, 10.0, dFdy(f)*iResolution.y ) );
    
    vec3 lig = normalize( vec3( 0.1, -0.9, -0.9 ) );
    float dif = clamp( 0.3+0.7*dot( nor, lig ), 0.0, 1.0 );
    vec3 bdrf;
    bdrf  = vec3(0.90,0.90,0.95)*(nor.y*0.5+0.5);
    bdrf += vec3(0.95,0.90,0.95)*dif;
    col *= 4.2*bdrf;
	col = 1.0-col;
